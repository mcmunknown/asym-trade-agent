...
class TradingEngine:
    ...
    async def process_signals(self, data_list: List[Dict]):
        ...
        try:
            ...
            async with self.glm_client:
                for symbol_data in data_list:
                    try:
                        # Apply complete prompt.md 7-category filter system
                        analysis_result = await self.glm_client.analyze_asymmetric_criteria(symbol_data)
                        ...
                        if analysis_result['signal'] == 'BUY':
                            ...
                            await self.handle_asymmetric_signal(symbol_data['symbol'], analysis_result, symbol_data)
                        else:
    ...
    async def handle_asymmetric_signal(self, symbol: str, analysis: Dict, symbol_data: Dict):
        ...
        try:
            if analysis['signal'] == 'BUY' and analysis['confidence'] >= 85:  # Higher confidence for asymmetric
                ...
                if signal:
                    await self.execute_asymmetric_trade(signal)
    ...
    async def execute_asymmetric_trade(self, signal: TradingSignal):
        ...
        try:
            ...
            async with self.bybit_client:
                ...
                leverage_set = await self.bybit_client.set_leverage(signal.symbol, signal.leverage)
                if not leverage_set:
                ...
                if order_result:
                    ...
                    tp_order = await self.bybit_client.place_order(
                        symbol=signal.symbol,
                        side='Sell',
                        order_type='Limit',
                        qty=signal.quantity,
                    ...
                    sl_order = await self.bybit_client.place_order(
                        symbol=signal.symbol,
                        side='Sell',
                        order_type='Market',
                        qty=signal.quantity,
            ...
            try:
                ...
                async with self.bybit_client:
                    ...
                    for symbol, position_data in self.active_positions.items():
                        ...
                        position_info = await self.bybit_client.get_position_info(symbol)
